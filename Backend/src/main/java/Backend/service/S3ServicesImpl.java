...
public class S3ServicesImpl implements S3Services {
    ...
    public void uploadS3File(String keyName, MultipartFile file) {
        try {
            ...
            String tailName = file.getOriginalFilename();
            if (tailName.substring(tailName.length()-3).equals("jpg")) {
                metadata.setContentType("image/jpg");
            } else if (tailName.substring(tailName.length()-3).equals("png")) {
                metadata.setContentType("image/png");
            } else {
                metadata.setContentType("image/jpeg");
            }
            // Can not only image upload
